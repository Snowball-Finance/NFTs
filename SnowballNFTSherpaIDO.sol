...
contract SnowballNFTSherpaIDO is ERC721, Ownable{
    ...
    uint256 public TOKEN_LIMIT = 2120;
    uint256 public MAX_TOKENS_PER_ADDRESS = 10; // increment this daily if necessary
    uint256 public _salePrice = 1200000000000000000; // 1 AVAX
    ...
    uint256 public _xSNOBRequired = 490e18; // 490 xSNOB, 10 xSNOB buffer for decay
    address payable _feeReceiver;
    mapping (address => uint256) private _mintCounts;
    
    IxSNOB public xSNOB;

    constructor() ERC721("Snowball NFT Sherpa IDO", "SNOBNFTSHERPAIDO") {
        _feeReceiver = payable(_feeAddress);
        setBaseURI("https://api.snowball.network/nft/sherpa_ido/");
        xSNOB = IxSNOB(_xSNOB);
    }

    function addressMintAvailable(address to) public view virtual returns (bool) {
        return _mintCounts[to] < MAX_TOKENS_PER_ADDRESS;
    }
    ...
    function updateTokenLimit(uint256 tokenLimit) public onlyOwner{
        TOKEN_LIMIT = tokenLimit;
    }
    
    function updateMaxPerAddress(uint256 maxTokens) public onlyOwner{
        MAX_TOKENS_PER_ADDRESS = maxTokens;
    }
    
    function updateSalePrice(uint256 price) public onlyOwner{
        _salePrice = price;
    }
